...
async function relationalQuery(
  ...
  return rawQuery(
    `
    select ${column} x,
      count(distinct website_event.visitor_id) as y
    from website_event
    ${cohortQuery}
    ${excludeBounceQuery}
    ${joinSessionQuery}
...
async function clickhouseQuery(
  ...
  if (
    ...
    hasSubHourTimezoneOffset(filters.timezone, filters.startDate)
  ) {
    let entryExitQuery = '';
    ...
    sql = `
    select ${column} x, 
      uniq(website_event.visitor_id) as y
    from website_event
    ${cohortQuery}
    ${excludeBounceQuery}
    ${entryExitQuery}
