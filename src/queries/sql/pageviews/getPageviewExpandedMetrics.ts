...
async function relationalQuery(
  ...
  if (type === 'entry' || type === 'exit') {
    ...
    entryExitQuery = `
      ...
      on x.session_id = website_event.session_id
  ...
  return rawQuery(
    ...
    select
      ...
      count(distinct t.session_id) as "visits",
    ...
    from (
      ...
      and website_event.event_type NOT IN (2, 5)
        ${excludeDomain}
        ${filterQuery}
      group by ${column}, website_event.visitor_id, website_event.session_id
    ) as t
    where name != ''
    group by name 
...
async function clickhouseQuery(
  ...
  return rawQuery(
    ...
    select
      name,
      sum(t.c) as "pageviews",
      uniq(t.visitor_id) as "visitors",
      uniq(t.session_id) as "visits",
    ...
    from (
      ...
      where website_id = {websiteId:UUID}
        ...
        ${excludeDomain}
        ${filterQuery}
      group by name, visitor_id, session_id
    ) as t
    group by name 
    order by visitors desc, visits desc
    limit ${limit}
