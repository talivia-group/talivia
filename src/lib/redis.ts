...
const log = debug('talivia:redis-client');
...
function getClient() {
  const redis = new TaliviaRedisClient(process.env.REDIS_URL);
...
const client: TaliviaRedisClient = globalThis[REDIS] || getClient();
