...
const log = debug('talivia:clickhouse');
...
function getCohortQuery(filters: Record<string, any>) {
  ...
  return `join (
      select distinct visitor_id as cohort_visitor_id
      from website_event
      ...
      on cohort.cohort_visitor_id = website_event.visitor_id
    `;
}
...
function getExcludeBounceQuery(filters: Record<string, any>) {
  ...
  return `join
    (select session_id as exclude_session_id
