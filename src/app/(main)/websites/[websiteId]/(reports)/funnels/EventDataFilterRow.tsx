...
import { useApi, useMessages, useMobile, useTimezone } from '@/components/hooks';
import { X } from '@/components/icons';

export function useEventDataDateRange() {
  const { fromUtc, toUtc, timezone, canonicalizeTimezone } = useTimezone();
  const today = fromUtc(new Date());

  return {
    startAt: +toUtc(subMonths(endOfDay(today), 6)),
    endAt: +toUtc(endOfDay(today)),
...
function PropertySelect({
  ...
  const { startAt, endAt } = useEventDataDateRange();
...
function ValueSelect({
  ...
  const { get, useQuery } = useApi();
  const { t, messages } = useMessages();
  const { startAt, endAt, timezone } = useEventDataDateRange();

  const { data, isLoading } = useQuery<Array<{ value: string; total: number }>>({
    queryKey: [
      'event-data:values',
      { websiteId, eventName, propertyName, startAt, endAt, timezone },
    ],
    queryFn: () =>
