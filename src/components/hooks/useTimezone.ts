...
import { getTimezone, parseUtcTimestamp } from '@/lib/date';
...
export function useTimezone() {
  ...
  const resolvedTimezone = TIMEZONE_LEGACY[timezone] ?? timezone;

  const formatTimezoneDate = (date: string | number | Date, pattern: string) => {
    return formatInTimeZone(parseUtcTimestamp(date), resolvedTimezone, pattern, {
      locale: dateLocale,
    });
  };
  ...
  const toUtc = (date: Date | string | number) => {
    return zonedTimeToUtc(date, resolvedTimezone);
  };

  const fromUtc = (date: Date | string | number) => {
    return utcToZonedTime(date, resolvedTimezone);
  };
