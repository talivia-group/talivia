...
export interface MetricLabelProps {
  ...
  data: any;
  onClick?: () => void;
  showKeywordSourceIcon?: boolean;
}

export function KeywordSourceIcon({
  sourceLabel,
}: {
  source?: string;
  sourceLabel?: string;
}) {
  const label = sourceLabel || 'Tracked keyword';

  return (
    <span className="talivia-keyword-source-icon" role="img" aria-label={label} title={label}>
      <span aria-hidden="true">UTM</span>
    </span>
  );
}
...
export function KeywordSourceLabel({
  ...
  const label = sourceLabel || 'UTM';

  return (
    <span className="talivia-keyword-source">
      <KeywordSourceIcon source={source} sourceLabel={sourceLabel} />
      <span className="talivia-keyword-source-text">{label}</span>
    </span>
  );
}

export function MetricLabel({ type, data, showKeywordSourceIcon = true }: MetricLabelProps) {
    ...
    case 'keywords':
      return (
        <FilterLink
          ...
          value={label}
          icon={
            showKeywordSourceIcon ? (
              <KeywordSourceIcon source={data.source} sourceLabel={data.sourceLabel} />
            ) : undefined
          }
        />
      );
