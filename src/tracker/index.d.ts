export type TrackedProperties = {
   ...
   * @example 'analytics.analytics.example.com'
   */
  hostname: string;
   ...
   *
   * @description extracted from `window.navigator.language`
   * @example 'https://app.example.com/docs/getting-started'
   */
  referrer: string;
...
export type TaliviaTracker = {
  track: {
     ...
     *
     * @example ```
     * talivia.track('signup-button', { name: 'newsletter', id: 123 });
     * ```
     */
     ...
     *
     * @example ```
     * talivia.track({ website: 'e676c9b4-11e4-4ef1-a4d7-87001773e9f2', url: '/home', title: 'Home page' });
     * ```
     */
     ...
     *
     * @example ```
     * talivia.track((props) => ({ ...props, url: path }));
     * ```
     */
    (eventFunction: CustomEventFunction): Promise<string>;
  };
  identify: (id: string | EventData, data?: EventData) => Promise<unknown>;
  getSessionId: () => string;
};

export interface Window {
  talivia: TaliviaTracker;
}
